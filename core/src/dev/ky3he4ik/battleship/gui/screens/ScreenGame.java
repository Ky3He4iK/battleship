...
public class ScreenGame extends BaseScreen {
    ...
    @Override
    public void act() {
        if (callback.turn == ScreensDirector.TURN_RIGHT) {
//            Gdx.app.debug("StepGame", "turn: right " + callback.aiReadyR + "; " + callback.canShoot(ScreensDirector.TURN_RIGHT) + "; " + callback.aiXR + 'x' + callback.aiYR);
            if (callback.canShoot(ScreensDirector.TURN_RIGHT)) {
                ...
                callback.nextTurn();
        } else if (callback.turn == ScreensDirector.TURN_LEFT) {
            if (callback.canShoot(ScreensDirector.TURN_LEFT)) {
    ...
    public void turnFinished(int playerId, int i, int j) {
        ...
        if (playerId == callback.turn && callback.canShoot(playerId)) {
            ...
            if (opponent.getWorld().isDead()) {
                ...
                && (callback.staticContent.config.getGameType() == GameConfig.GameType.AI
                || callback.staticContent.config.getGameType() == GameConfig.GameType.AI_VS_AI)) {
            Gdx.app.debug("StepGame", "set right turn");
            callback.aiReadyR = true;
            callback.aiXR = i;
            callback.aiYR = j;
        } else if (playerId == ScreensDirector.TURN_LEFT && callback.staticContent.config.getGameType() == GameConfig.GameType.AI_VS_AI) {
